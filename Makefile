...
#   /usr/bin/make pack     # /workspace/xmp-pokey-1.0.5.zip
#
...
$(OBJ)/asap.o: $(ASAP)/asap.c $(SRC)/asap_xmp_tail.c $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	cat $(ASAP)/asap.c $(SRC)/asap_xmp_tail.c | $(CC) $(CFLAGS_L) -x c -c -o $@ -

$(OBJ)/pokey_player.o: $(SRC)/pokey_player.c $(SRC)/pokey_player.h $(SRC)/pokey_scan.h $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS_L) -c -o $@ $<

$(OBJ)/pokey_scan.o: $(SRC)/pokey_scan.c $(SRC)/pokey_scan.h $(SRC)/pokey_player.h $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS_L) -c -o $@ $<

$(OBJW)/asap.o: $(ASAP)/asap.c $(SRC)/asap_xmp_tail.c $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	cat $(ASAP)/asap.c $(SRC)/asap_xmp_tail.c | $(I686_CC) $(CFLAGS_W) -x c -c -o $@ -

$(OBJW)/pokey_player.o: $(SRC)/pokey_player.c $(SRC)/pokey_player.h $(SRC)/pokey_scan.h $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	$(I686_CC) $(CFLAGS_W) -c -o $@ $<

$(OBJW)/pokey_scan.o: $(SRC)/pokey_scan.c $(SRC)/pokey_scan.h $(SRC)/pokey_player.h $(ASAP)/asap.h
	@mkdir -p $(dir $@)
	$(I686_CC) $(CFLAGS_W) -c -o $@ $<

$(DIST)/test_asap_render: $(ROOT)/tests/test_asap_render.c $(OBJ)/asap.o $(OBJ)/pokey_player.o $(OBJ)/pokey_scan.o
	mkdir -p $(DIST)
	$(CC) $(CFLAGS_L) -o $@ $(ROOT)/tests/test_asap_render.c \
	  $(OBJ)/asap.o $(OBJ)/pokey_player.o $(OBJ)/pokey_scan.o -lm
...
$(DIST)/xmp-pokey.dll: $(SRC)/xmp-pokey.cpp $(SRC)/xmp-pokey.def $(OBJW)/asap.o $(OBJW)/pokey_player.o $(OBJW)/pokey_scan.o $(OBJW)/xmp-pokey.res
	mkdir -p $(DIST)
	$(I686_CXX) -shared -O2 -DNDEBUG -std=c++14 \
	  -static -static-libgcc -static-libstdc++ \
	  -I$(INC) -I$(SRC) -I$(ASAP) -DWIN32 -D_WIN32 \
	  -o $@ $(SRC)/xmp-pokey.cpp $(SRC)/xmp-pokey.def \
	  $(OBJW)/asap.o $(OBJW)/pokey_player.o $(OBJW)/pokey_scan.o $(OBJW)/xmp-pokey.res \
...
pack: dll
	rm -f /workspace/xmp-pokey-1.0.5.zip
	mkdir -p $(DIST)/pack
	cp -f $(DIST)/xmp-pokey.dll $(ROOT)/README.md $(DIST)/pack/
	cd $(DIST)/pack && zip -9 /workspace/xmp-pokey-1.0.5.zip xmp-pokey.dll README.md
	rm -rf $(DIST)/pack
	ls -l /workspace/xmp-pokey-1.0.5.zip
