...
#   /usr/bin/make pack     # /workspace/xmp-pokey-1.0.zip
#
...
pack: dll
	rm -f /workspace/xmp-pokey-1.0.zip
	mkdir -p $(DIST)/pack
	cp -f $(DIST)/xmp-pokey.dll $(ROOT)/README.md $(DIST)/pack/
	cd $(DIST)/pack && zip -9 /workspace/xmp-pokey-1.0.zip xmp-pokey.dll README.md
	rm -rf $(DIST)/pack
	ls -l /workspace/xmp-pokey-1.0.zip
